...
local_staging_start_core_services() {
  ...
  local integrate_shas=""
  for repo in pagayo-storefront pagayo-api-stack pagayo-config pagayo-schema; do
    local rp="$ws/$repo"
    if [[ -d "$rp/.git" ]] && git -C "$rp" show-ref --verify --quiet refs/heads/local/staging 2>/dev/null; then
      local short
      short="$(git -C "$rp" rev-parse --short local/staging 2>/dev/null || true)"
      [[ -n "$short" ]] && integrate_shas="${integrate_shas}${repo}@${short};"
    fi
  done
