...
function isSuccessResponse(data: unknown): data is SuccessResponse<unknown> {
  return typeof data === 'object' && data !== null && 'success' in data && (data as SuccessResponse<unknown>).success === true;
}

function isErrorResponse(data: unknown): data is ErrorResponse {
  return typeof data === 'object' && data !== null && 'success' in data && (data as ErrorResponse).success === false;
}

// Helper for AI-readable output
function logResult(test: string, status: 'PASS' | 'WARN' | 'FAIL', details: string) {
...
describe('Contract Tests - Health Endpoints', () => {
  it('beheer /api/health should match health schema', async () => {
    ...
    if (contentType.includes('text/html')) {
      logResult('beheer-health', 'WARN', 'Returns HTML (Cloudflare Access) - skipping JSON validation');
...
describe('Contract Tests - Success Responses', () => {
  it('/api/capabilities/features should return success response', async () => {
    ...
    const contentType = response.headers.get('content-type') || '';
    if (contentType.includes('text/html')) {
      logResult('capabilities-features', 'WARN', 'Returns HTML (Cloudflare Access)');
      expect(response.status).toBe(200);
      return;
    }
    ...
    const hasData = isSuccessResponse(data) || (data.features && Array.isArray(data.features));
    
    if (hasData) {
      const count = data.data?.length || data.features?.length || 0;
      logResult('capabilities-features', 'PASS', `${count} features returned`);
    } else {
      logResult('capabilities-features', 'FAIL', `Unexpected response format: ${JSON.stringify(data).slice(0, 100)}`);
    }
...
describe('Contract Tests - Error Responses', () => {
  it('validation errors should match error schema', async () => {
    ...
    if (response.status === 400) {
      ...
      logResult('validation-error', hasMessage ? 'PASS' : 'WARN', `status=${response.status}, hasMessage=${!!hasMessage}`);
      expect(hasMessage).toBeDefined();
    } else if (response.status === 429) {
      logResult('validation-error', 'WARN', 'Rate limited (429) - cannot test validation');
    }
  });
  ...
  it('auth errors should match error schema', async () => {
    ...
    if (response.status === 200) {
      ...
      if (ct.includes('text/html')) {
        logResult('auth-error', 'PASS', 'Protected by Cloudflare Access (login page)');
        return;
      }
    }
  ...
  it('404 errors should match error schema', async () => {
    ...
    if (response.status === 404) {
      ...
      logResult('404-error', isError ? 'PASS' : 'WARN', `status=404, matchesSchema=${isError}`);
    } else {
...
describe('Contract Tests - Features Schema', () => {
  ...
  it('/api/capabilities/features should return valid feature objects', async () => {
    ...
    if (features.length > 0) {
      ...
      const hasIdentifier = typeof feature.feature === 'string' || typeof feature.code === 'string';
      const hasName = typeof feature.name === 'string';
      logResult('feature-schema', hasIdentifier && hasName ? 'PASS' : 'FAIL', 
