...
describe('Infrastructure - DNS Resolution', () => {
  test.each(Object.entries(DOMAINS))('%s DNS resolves', (name, domain) => {
    ...
    if (resolves) {
      ...
      log(`dns-${name}`, 'FAIL', `${domain} DNS failure`,
        'Check Cloudflare DNS records', 'CRITICAL');
    }
...
describe('Infrastructure - SSL Certificates', () => {
  test.each(Object.entries(DOMAINS))('%s SSL valid', (name, domain) => {
    ...
    if (daysUntilExpiry === null) {
      log(`ssl-${name}`, 'FAIL', `${domain} SSL check failed`,
        'Check SSL/TLS settings in Cloudflare', 'CRITICAL');
    } else if (daysUntilExpiry <= 7) {
      log(`ssl-${name}`, 'FAIL', `${domain} SSL expires in ${daysUntilExpiry} days!`,
        'URGENT: SSL renewal needed', 'CRITICAL');
    } else if (daysUntilExpiry <= 30) {
      log(`ssl-${name}`, 'WARN', `${domain} SSL expires in ${daysUntilExpiry} days`,
        'Monitor SSL renewal (auto-renew should handle)');
    } else {
      log(`ssl-${name}`, 'PASS', `${domain} SSL valid for ${daysUntilExpiry} days`);
    }
...
describe('Infrastructure - HTTPS Reachability', () => {
  test.each(Object.entries(DOMAINS))('%s HTTPS reachable', async (name, domain) => {
    ...
    if (response.status < 600) {
      log(`https-${name}`, 'PASS', `${domain} reachable (HTTP ${response.status})`);
    } else {
      log(`https-${name}`, 'FAIL', `${domain} unreachable`,
        'Check Cloudflare and origin server', 'CRITICAL');
    }
...
describe('Infrastructure - Cloudflare Headers', () => {
  it('beheer.pagayo.com has CF headers', async () => {
    ...
    if (hasCF) {
      log('cf-headers-beheer', 'PASS', `cf-ray: ${response.headers.get('cf-ray')}`);
    } else {
      log('cf-headers-beheer', 'FAIL', 'Missing cf-ray header',
        'Check Cloudflare proxy status (orange cloud)', 'HIGH');
    }
  ...
  it('www.pagayo.com has CF headers', async () => {
    ...
    if (hasCF) {
      log('cf-headers-www', 'PASS', `cf-ray: ${response.headers.get('cf-ray')}`);
    } else {
      log('cf-headers-www', 'FAIL', 'Missing cf-ray header',
        'Check Cloudflare proxy status', 'HIGH');
    }
...
describe('Infrastructure - Critical Routing', () => {
  it('app.pagayo.com routes to Pages', async () => {
    ...
    if (response.status === 200) {
      ...
      log('routing-app', 'FAIL', `HTTP ${response.status}`,
  ...
  it('beheer.pagayo.com routes to Worker', async () => {
    const response = await fetch(`https://${DOMAINS.beheer}/api/health`, { redirect: 'follow' });
    
    if (response.status === 200) {
      log('routing-beheer', 'PASS', 'beheer.pagayo.com → Worker');
    } else {
      log('routing-beheer', 'FAIL', `HTTP ${response.status}`,
        'Check Beheer Worker deployment', 'CRITICAL');
    }
  ...
  it('*.pagayo.app routes to Storefront Worker', async () => {
    ...
    if (response.status === 200) {
      ...
      log('routing-storefront', 'FAIL', `HTTP ${response.status}`,
