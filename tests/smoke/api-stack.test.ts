  ...
  describe('Health Endpoints', () => {
    it('API health endpoint returns 200', async () => {
      ...
      if (response.status === 200 && data) {
        ...
        log('api-health', 'FAIL', `HTTP ${response.status}`,
          'Check API Stack Worker deployment', 'CRITICAL');
    ...
    it('Root endpoint returns 200', async () => {
      ...
      if (response.status === 200) {
        ...
        log('root', 'FAIL', `HTTP ${response.status}`,
          'Check Worker routing', 'HIGH');
      }
  ...
  describe('Protected Endpoints', () => {
    it('Orders endpoint requires auth', async () => {
      ...
      if ([401, 403, 404].includes(response.status)) {
        ...
        log('orders-auth', 'FAIL', `Server error: HTTP ${response.status}`,
          'Check orders endpoint handler', 'HIGH');
      }
  ...
  describe('Webhook Endpoints', () => {
    it('Stripe webhook rejects invalid signature', async () => {
      ...
      if ([400, 401, 404].includes(response.status)) {
        log('stripe-webhook', 'PASS', `Signature validation: HTTP ${response.status}`);
      } else if (response.status >= 500) {
        log('stripe-webhook', 'FAIL', 'Webhook handler crash',
          'Check Stripe webhook handler', 'HIGH');
      }
    ...
    it('Mollie webhook handles test payload', async () => {
      ...
      if ([200, 400, 401, 404].includes(response.status)) {
        ...
        log('mollie-webhook', 'FAIL', 'Webhook handler crash',
          'Check Mollie webhook handler', 'HIGH');
      }
