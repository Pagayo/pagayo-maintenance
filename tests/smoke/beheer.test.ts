...
describe("Beheer Service - Smoke Tests", () => {
  describe("Health Endpoints", () => {
    it("API health endpoint returns 200", async () => {
      ...
      if (response.status === 200 && contentType.includes("application/json")) {
        ...
        log(
          "api-health",
          ["healthy", "ok"].includes(data.status) ? "PASS" : "WARN",
          `Status: ${data.status}`,
        );
        ...
        log(
          "api-health",
          "WARN",
          "Returns HTML (CF Access redirect)",
    ...
    it("Root health endpoint returns build info", async () => {
      ...
      if (response.status === 200 && contentType.includes("application/json")) {
        ...
        log(
          "health",
          data.status && data.build ? "PASS" : "WARN",
  ...
  describe("Public Routes (Cloudflare Access Bypass)", () => {
    it("Registration NOT blocked by Cloudflare Access", async () => {
      ...
      if (response.status === 403) {
        log(
          "register-bypass",
          "FAIL",
          "CF Access blocking registration",
          "Add /api/auth/* to bypass policy",
          "CRITICAL",
        );
      } else {
    ...
    it("Workflows NOT blocked by Cloudflare Access", async () => {
      const response = await fetch(
        `${BEHEER_URL}/api/workflows/provisioning/status/test`,
      );
      ...
      if (response.status === 403) {
        log(
          "workflows-bypass",
          "FAIL",
          "CF Access blocking workflows",
          "Add /api/workflows/* to bypass policy",
          "CRITICAL",
        );
      } else {
        log(
          "workflows-bypass",
          "PASS",
          `HTTP ${response.status} - Not blocked`,
        );
      }
    ...
    it("Capabilities endpoint returns features", async () => {
      ...
      if (response.status === 200) {
        ...
        expect(data.success === true || Array.isArray(data.features)).toBe(
          true,
        );
      } else {
        log(
  ...
  describe("Protected Routes (Auth Required)", () => {
    ...
    it("Tenants endpoint requires auth (no crash)", async () => {
      const response = await fetch(`${BEHEER_URL}/api/tenants`, {
        redirect: "manual",
      });
    ...
    it("Full registration provisions tenant successfully", async () => {
      ...
      if (response.status >= 500) {
        log(
          ...
          "FAIL",
          `Server error: HTTP ${response.status}`,
          "Check Worker logs and database schema sync",
          "CRITICAL",
        );
        expect(response.status).toBeLessThan(500);
        return;
        ...
        if (response.status === 202 && data.async === true) {
          if (!data.workflowId) {
            log(
              "register-provision",
              "FAIL",
              "Async response but no workflowId",
              "Check workflow trigger logic",
              "CRITICAL",
            );
          ...
          for (let attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            if (
              ...
              log(
                ...
                "Workflow status requires auth - manual verification needed",
                "MEDIUM",
              );
              return;
            }
            ...
            if (
              ...
              const errorMsg =
                statusData.error || statusData.data?.error || "unknown";
              log(
                "register-provision",
                "FAIL",
                `Workflow FAILED: ${errorMsg}`,
                "Check Neon DB schema sync - run prisma migrate deploy",
                "CRITICAL",
              );
          ...
          log(
            ...
            "Workflow taking too long - check manually",
            "MEDIUM",
          );
          return;
        }
        ...
        if (data.success === true) {
          ...
          if (data.tenantProvisioned === true || data.tenant) {
            ...
            log(
              ...
              "Add tenantProvisioned flag to response",
              "HIGH",
            );
          }
        } else {
    ...
    it("POST /api/anon/create-account handles empty body", async () => {
      ...
      if ([200, 201, 202].includes(response.status)) {
        ...
        log(
          "anon-create-account",
          "PASS",
          `Properly validates input: HTTP ${response.status}`,
        );
      } else if (response.status >= 500) {
        log(
          "anon-create-account",
          "FAIL",
          `Server error: HTTP ${response.status}`,
          "Check anon.routes.ts create-account",
          "HIGH",
        );
      } else {
        log(
    ...
    it("POST /api/anon/create-order validates input", async () => {
      ...
      if ([400, 401].includes(response.status)) {
        ...
        log(
          ...
          "FAIL",
          `Server error: HTTP ${response.status}`,
          "Check anon.routes.ts create-order",
          "HIGH",
        );
      }
    ...
    it("GET /api/anon/account/nonexistent does not crash", async () => {
      ...
      if (response.status === 404) {
        ...
        log(
          ...
          "FAIL",
          `Server error: HTTP ${response.status}`,
          "Check anon.routes.ts account/:slug",
          "HIGH",
        );
      } else {
        log(
    ...
    it("POST /api/anon/passkey/register does not crash", async () => {
      ...
      if ([400, 401].includes(response.status)) {
        ...
        log(
          ...
          "FAIL",
          `Server error: HTTP ${response.status}`,
          "Check passkey register endpoint",
          "HIGH",
        );
      }
    ...
    it("POST /api/anon/passkey/authenticate does not crash", async () => {
      ...
      if ([400, 401].includes(response.status)) {
        ...
        log(
          ...
          "FAIL",
          `Server error: HTTP ${response.status}`,
          "Check passkey authenticate endpoint",
          "HIGH",
        );
      }
